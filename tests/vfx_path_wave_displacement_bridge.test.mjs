...
import {
  waveDisplacedPathSetToAxmScene,
  observeVisualEffectPathWaveDisplacement,
} from "../src/vfx_path_wave_displacement_bridge.mjs";

function fixture(overrides = {}) {
  return {
    schema: "axm.wave-displaced-path-set/v0.1",
    displacementSourceHash: "wave-source",
...
test("path-wave scene adapter is deterministic, bounded and constant styled", () => {
  const first = waveDisplacedPathSetToAxmScene(fixture());
  const second = waveDisplacedPathSetToAxmScene(structuredClone(fixture()));
...
test("path-wave bridge rejects non-exact donor revisions before donor access", async () => {
  await assert.rejects(
    observeVisualEffectPathWaveDisplacement("/definitely/not/a/donor", { vfxRevision: "main" }),
    /exact 40-character lowercase git SHA/,
  );
});

test("path-wave adapter fails closed on promoted authority and degenerate segments", () => {
  assert.throws(
    () => waveDisplacedPathSetToAxmScene(fixture({ derived: false })),
    /derived and rebuildable/,
  );
  const degenerate = fixture();
  degenerate.paths[0].points[1] = structuredClone(degenerate.paths[0].points[0]);
  assert.throws(
    () => waveDisplacedPathSetToAxmScene(degenerate),
    /degenerate segment/,
  );
});
