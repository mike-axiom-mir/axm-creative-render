...
function parseArgs(argv) {
  ...
  for (const key of ["uc-root", "base", "effect", "output", "receipt", "render-request", "render-receipt", "vfx-receipt", "vfx-svg", "rasterizer-evidence"]) {
...
try {
  ...
  const [baseBytes, effectBytes, renderRequest, renderReceipt, vfxReceiptBytes, vfxSvg, rasterizerEvidence] = await Promise.all([
    readFile(basePath),
    ...
    readFile(resolve(args["rasterizer-evidence"])),
  ]);
  ...
  const receipt = {
    ...
    version: 1,
    mode: "vfx-source-to-raster-to-universal-creation-screen-composite",
    upstream_render: {
    ...
    vfx_source: {
      ...
      rasterizer_evidence_sha256: sha256(rasterizerEvidence),
    },
    ...
    truth_boundary: {
      proves: [
        ...
        "an explicitly evidenced external rasterizer materialized that SVG into a bounded PPM effect frame",
      ...
      does_not_prove: [
        ...
        "the external SVG rasterizer is deterministic across machines",
