...
import { buildUcMixedFabricationSceneSet } from "./uc_mixed_fabrication_bridge.mjs";
...
function parseArgs(argv) {
  ...
  for (const key of ["uc-root","donor-bundle","source-scene","stage-one-scene","stage-two-scene","evidence"]) if (!values[key]) throw new Error(`missing --${key}`);
  return values;
}
...
try {
  ...
  const ucRoot = resolve(args["uc-root"]), bundlePath = resolve(args["donor-bundle"]);
  const sourceScenePath = resolve(args["source-scene"]), stageOneScenePath = resolve(args["stage-one-scene"]), stageTwoScenePath = resolve(args["stage-two-scene"]), evidencePath = resolve(args.evidence);
  const outputs = [sourceScenePath,stageOneScenePath,stageTwoScenePath,evidencePath];
  if (new Set(outputs).size !== outputs.length) throw new Error("mixed fabrication outputs must be distinct");
  for (const path of outputs) if (pathIsInside(ucRoot,path)) throw new Error("mixed fabrication outputs may not be written inside the Universal Creation donor repository");
  if (pathIsInside(ucRoot,bundlePath)) throw new Error("mixed fabrication donor bundle must remain outside the Universal Creation repository");

  const bundleBytes = await readFile(bundlePath);
  const result = buildUcMixedFabricationSceneSet(bundleBytes, { albedo:[82,180,210] });
  ...
  const evidence = {
    contract:"AXM_CREATIVE_UC_MIXED_FABRICATION_SCENE_EVIDENCE",version:1,
  ...
  console.log("uc_mixed_fabrication_bridge=PASS");
  ...
  console.log(`operations=${result.observation.donor.stage_one_operations.join('+')} -> ${result.observation.donor.stage_two_operations.join('+')}`);
